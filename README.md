...
- Tailwind CSS with a custom Japandi-inspired design system — warm off-white palette, Georgia serif headings, editorial section labels
...
### Data Integrity Fix
During development, identified that 8 ingredient IDs referenced by recipes in the provided data.json had no corresponding entries in the ingredients lookup table, causing blank ingredient names on the detail page. Added the missing ingredient definitions (basil, butter, brown_sugar, white_sugar, broccoli, carrot, soy_sauce, ginger) with realistic nutritional values to resolve the join issue. Also added missing cuisine tags to four recipes (Chocolate Chip Cookies, Berry Smoothie Bowl, Quinoa Buddha Bowl, Almond-Crusted Chicken).
...
- A proper database (PostgreSQL or SQLite) replacing the JSON mock
