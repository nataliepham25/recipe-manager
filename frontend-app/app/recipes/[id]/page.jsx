...
function MetaChip({ label, children }) {
  return (
    ...
    >
      <span className="text-[9px] uppercase tracking-wide text-text-muted mb-0.5">
        {label}
      </span>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6">
        {[...Array(4)].map((_, i) => (
          <Bone key={i} className="flex-1 h-14 rounded-lg" />
        ))}
      </div>
      <div className="flex gap-2 mb-8">
        {[14, 12, 18].map((w) => (
          <Bone key={w} className={`h-4 w-${w} rounded-full`} />
        ))}
      </div>
  ...
  const fetchRecipe = useCallback(() => {
    ...
    fetch(`${API}/recipes/${id}`)
      .then(async (r) => {
        ...
        if (!json.success)
          throw new Error(json.error || "Failed to load recipe");
        setRecipe(json.data);
      ...
      <PageWrapper>
        ...
        {!loading && notFound && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <Link href="/recipes" className={actionBtn}>
              ← All recipes
            </Link>
          </div>
        )}
        ...
        {!loading && error && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button onClick={fetchRecipe} className={actionBtn}>
              Try again
            </button>
          </div>
        )}
        ...
        {!loading &&
          recipe &&
          (() => {
            const cuisineTag = recipe.tags?.find((t) =>
              KNOWN_CUISINES.has(t.toLowerCase()),
            );
            const eyebrow = [
              cuisineTag &&
            ...
            return (
              <>
                ...
                {/* Description */}
                {recipe.description && (
                  <p className="text-sm text-text-secondary mt-[6px] mb-4">
                    {recipe.description}
                  </p>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
                  ...
                  <div className="lg:col-span-3">
                    <h2 className="text-sm font-medium text-text-primary mb-5">
                      Instructions
                    </h2>
                    <ol className="space-y-5">
                      {recipe.instructions.map((step, i) => (
                        <li key={i} className="flex gap-4">
                          ...
                          <p
                            className="text-sm text-text-primary pt-1"
                    ...
                    <ul className="mb-6">
                      {recipe.ingredients.map((ing, i) => {
                        const scaled = formatAmount(
                          (parseAmount(ing.amount) * servings) /
                            recipe.servings,
                        );
                        return (
                          <li
                ...
                >
                  ...
                  <div className="flex items-start gap-2 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs" style={{ color: "#7A6355" }}>
                        Ask Claude about this recipe
                      </p>
                    </div>
                  </div>
                      ...
                      {aiLoading && (
                        <div className="space-y-2">
                          <div
                            ...
                            style={{ background: "#E5DDD3", width: "80%" }}
                          />
                          <div
                            className="h-3 rounded animate-pulse"
                            style={{ background: "#E5DDD3", width: "60%" }}
                          />
                        </div>
                      )}
